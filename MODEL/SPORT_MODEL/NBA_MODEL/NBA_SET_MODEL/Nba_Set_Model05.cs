

namespace EASY_CALLENDER01.MODEL.SPORT_MODEL.NBA_MODEL.NBA_SET_MODEL
{
    internal class Nba_Set_Model05
    {
        public string name { get; set; }= string.Empty;
        public int rank { get; set; }
        public int win { get; set; }
        public int loss { get; set; }
        public string name1 { get; set; } = string.Empty;
        public int rank1 { get; set; }
        public int win1 { get; set; }
        public int loss1 { get; set; }
        public string gamesBehind { get; set; } = string.Empty;
        public int home { get; set; }
        public int away { get; set; }
        public int total { get; set; }
        public string percentage { get; set; } = string.Empty;
        public int lastTen { get; set; }
        public string league { get; set; }= string.Empty;   
        public string season { get; set; } = string.Empty;
        public string league1 { get; set; } = string.Empty;
        public int season1 { get; set; }
        public string gamesBehind1 { get; set; } = string.Empty;
        public int streak { get; set; }
        public bool winStreak { get; set; }
        public object tieBreakerPoints { get; set; }=string.Empty;
        public string get { get; set; } = string.Empty;
        public object errors { get; set; } = string.Empty;
        public int results { get; set; }
    }
}
