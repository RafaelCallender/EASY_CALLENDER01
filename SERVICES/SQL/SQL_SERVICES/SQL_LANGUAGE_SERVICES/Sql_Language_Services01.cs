...
using System.Data;
using EASY_CALLENDER01.SERVICES.SQL_SERVICES.SQL.SQL_MANAGER.SQL_MANAGER_LANGUAGE;
namespace EASY_CALLENDER01.SERVICES.SQL.SQL_SERVICES.SQL_LANGUAGE_SERVICES
{
    public class Sql_Language_Services01
    {
        ...
        public bool find_language_using_code(string input, out string output)
        {
            ...
            using (SqlDataReader reader = Sql_Manager01.cmd[(int)Sql_Manager01.command_strings.find_language_using_code].ExecuteReader())
            {
                if (reader.Read())
                {
        ...
        public bool find_language_using_Language(string input, out string output)
        {
            ...
            using (SqlDataReader reader = Sql_Manager01.cmd[(int)Sql_Manager01.command_strings.find_language_using_Language].ExecuteReader())
            {
                if (reader.Read())
                {
                    output = $"{reader["language"].ToString()}";
        ...
        public string view_all_data_from_language_table()
        {
         ...
         public bool view_languge_database_storedprocedures(out string output)
        {

            output = string.Empty;
           Sql_Manager01.conn[(int)Sql_Manager01.Connection_strings.Connection01].Open();
            Sql_Manager01.cmd[(int)Sql_Manager01.command_strings.view_languge_database_storedprocedures].CommandType = CommandType.Text;
            using (SqlDataReader reader = Sql_Manager01.cmd[(int)Sql_Manager01.command_strings.view_languge_database_storedprocedures].ExecuteReader())
            {
                if (reader.Read())
                {
                    while (reader.Read())
