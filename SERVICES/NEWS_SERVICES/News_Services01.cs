...
namespace EASY_CALLENDER01.SERVICES.NEWS_SERVICES
{
    public class News_Services01
    {
        ...
        public async Task<string> Real_Time_News_Data_GET_Search(string input)
        {
            ...
            data01[2] = await news_data01($"https://real-time-news-data.p.rapidapi.com/search?query={input.Trim()}&limit=10&time_published=anytime&country=US&lang={get_language}");
            return data01[2];
        }
        public async Task<string> Real_Time_News_Data_GET_Top_Headlines()
        {

            data01[1] = await news_data02($"https://real-time-news-data.p.rapidapi.com/top-headlines?limit=500&country=US&lang={get_language}");
            return data01[1];
        }
        ...
        public async Task<string> Local_Headlines_Geo(string input)
        {
            ...
            data01[2] = await news_data01($"https://real-time-news-data.p.rapidapi.com/local-headlines?query={input.Trim()}&country=US&lang={get_language}&limit=500");
            return data01[2];
        }
        public async Task<string> Full_Story_Coverage()
        {

            data01[2] = await news_data03($"https://real-time-news-data.p.rapidapi.com/full-story-coverage?story=CAAqNggKIjBDQklTSGpvSmMzUnZjbmt0TXpZd1NoRUtEd2pzbFA3X0N4RjlDUlpVVnhudXBpZ0FQAQ&sort=RELEVANCE&country=US&lang={get_language}");
            return data01[2];
        }
