using EASY_CALLENDER01.MODEL.NEWS_MODEL.NEWS_GET_MODEL;
using EASY_CALLENDER01.MODEL.NEWS_MODEL.NEWS_SET_MODEL;
using EASY_CALLENDER01.SERVICES.FILE_SERVICES.TEXT_FILES;
using Newtonsoft.Json;


namespace EASY_CALLENDER01.SERVICES.NEWS_SERVICES
{
    public class News_Services01
    {
		...
		private string get_language="en";
        ...
        public async Task<string> Real_Time_News_Data_GET_Search(string input)
        {
             ...
             return  await news_data01($"https://real-time-news-data.p.rapidapi.com/search?query={input.Trim()}&limit=10&time_published=anytime&country=US&lang={get_language}");
            
        }
        public async Task<string> Real_Time_News_Data_GET_Top_Headlines()
        {

         return await news_data02($"https://real-time-news-data.p.rapidapi.com/top-headlines?limit=500&country=US&lang={get_language}");
        
        }
        public async Task<string> Real_Time_News_Data_GET_Topic_Headlines()
        {

            data01[1] = await news_data02($"https://real-time-news-data.p.rapidapi.com/topic-headlines?topic=WORLD&limit=500&country=US&lang={get_language}");
            return data01[1];
        }
        public async Task<string> Real_Time_News_Data_GET_Topic_News_By_Section(string input)
        {

        return await news_data02($"https://real-time-news-data.p.rapidapi.com/topic-news-by-section?topic={input.Trim()}&section=CAQiSkNCQVNNUW9JTDIwdk1EZGpNWFlTQldWdUxVZENHZ0pKVENJT0NBUWFDZ29JTDIwdk1ETnliSFFxQ2hJSUwyMHZNRE55YkhRb0FBKi4IACoqCAoiJENCQVNGUW9JTDIwdk1EZGpNWFlTQldWdUxVZENHZ0pKVENnQVABUAE&limit=500&country=US&lang={get_language}");
    ...
    return await news_data01($"https://real-time-news-data.p.rapidapi.com/local-headlines?query={input.Trim()}&country=US&lang={get_language}&limit=500");
            
        }
        public async Task<string> Full_Story_Coverage()
        {

            return await news_data03($"https://real-time-news-data.p.rapidapi.com/full-story-coverage?story=CAAqNggKIjBDQklTSGpvSmMzUnZjbmt0TXpZd1NoRUtEd2pzbFA3X0N4RjlDUlpVVnhudXBpZ0FQAQ&sort=RELEVANCE&country=US&lang={get_language}");
            
        }
        public async Task<string> Real_Time_News_Data_GET_Language_List()
        {

            return await news_data04($"https://real-time-news-data.p.rapidapi.com/language-list?country=US");
